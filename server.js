...
try {
  ...
  app.get("/api/verify-email", async (req, res) => {
    ...
    try {
      ...
      // Check if Nelliel already exists for this user
const existing = await prisma.character.findFirst({
  where: { userId: user.id, name: "Nelliel" }
});
...
if (!existing) {
  ...
  await prisma.userPreference.upsert({
    ...
    update: { selectedCharId: nelliel.id },
    create: { userId: user.id, selectedCharId: nelliel.id }
  });
}
